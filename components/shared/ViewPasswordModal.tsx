...
import { ViewPasswordModalProps } from "@/types";
import { passwordService } from "@/services/api";

export function ViewPasswordModal({ open, onOpenChange, passwordEntry }: ViewPasswordModalProps) {
  ...
  const handleDecrypt = async (passwordId: string, masterPassword: string) => {
    ...
    setError("");
    try {
      const result = await passwordService.decryptPassword({
        passwordId,
        masterPassword,
      });
      
      if (result.success && result.password) {
        setDecrypted(result.password);
        setTimer(20);
        return result.password;
      } else {
        setError(result.error || "Unable to fetch at this moment. Try again");
      }
    } catch (err: any) {
